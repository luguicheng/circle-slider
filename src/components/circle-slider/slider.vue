...
export default {
  ...
  mounted() {
    ...
    const sliderTolerance = n < this.cpPathStrokeWidth ? this.cpPathStrokeWidth : n
  ...
  methods: {
    ...
    updateAngle(angle) {
      ...
      if (angle == 0 && this.pos == 'end') {
        this.circleSliderState.updateCurrentStepFromAngle(2 * Math.PI, this.pos)
      } else {
    ...
    updateFromPropValue(value) {
      this.$nextTick(() => {
        ...
        this.circleSliderState.updateCurrentStepFromValue(lastStepValue, 'end')
